...
/// Creates a configuration struct that is `pub(crate)`.
...
/// Creates a public configuration struct.
/// Define field names and types for configurable variables.
/// ```
/// ezcfg::pub_cfg!{ 
///     TestConfig ["~/test/path.cfg"] 
...
macro_rules! pub_cfg {
    ($name:ident [$path:expr] $($field:ident: $typ:ident),* $(,)?) => {
        ...
        impl $name {
            pub fn new($($field: $typ,)*) -> Self { Self {$( $field, )*}}
            $(pub fn $field(&self) -> &$typ { &self.$field })*
        }
